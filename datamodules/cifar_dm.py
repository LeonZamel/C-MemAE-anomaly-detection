...
class CIFARDataModule(pl.LightningDataModule):
    ...
    @staticmethod
    def target_name_map():
        # Returns a mapping from numbers 0-9 to the corresponding class names
        return dict(
            list(
                enumerate(
                    [
    ...
    def setup(self, stage=None):
        dataset_train = datasets.CIFAR10(self.data_dir, train=True, download=False, transform=self.transform)
        dataset_test = datasets.CIFAR10(self.data_dir, train=False, download=False, transform=self.transform)
